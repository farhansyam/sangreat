        ...
        <script type="module">
            app.component('v-checkout', {
                ...
                methods: {
                    getOrderSummary() {
                        this.$axios.get("{{ route('shop.checkout.onepage.summary') }}")
                            .then(response => {
                                ...
                                this.idKota = this.cart.billing_address.id_kota; // Simpan idKota dalam variabel data
                    ...
                    getCost() {
                        this.$axios.get("{{ route('shop.api.core.cost') }}", {
                                params: {
                                    id_city: this.idKota // Gunakan this.idKota yang sudah disimpan sebelumnya
                                }
                            })
                            .then(response => {
                                this.cost = response.data.cost[0]['costs'];
                                this.shippingMethods = response.data.shippingMethods; // Pastikan ini sesuai dengan struktur yang diharapkan
                                console.log(this.cost)
                                // Lakukan sesuatu dengan this.cost
                            })
